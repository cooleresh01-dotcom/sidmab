...
export default function Footer() {
  ...
  const siteName = settings?.siteName || 'SIDMAB'
  const tagline = settings?.tagline || 'Events & Management'
        ...
        <>
          <ul className="space-y-2">
            <li className="flex items-start gap-2">
              ...
              <span className="text-sm text-white/50 leading-relaxed">{settings?.address || '123 Event Street, Lagos, Nigeria'}</span>
            </li>
            <li className="flex items-center gap-2">
              ...
              <a href={`tel:${(settings?.phone || '+234 800 000 0000').replace(/\s/g, '')}`} className="text-sm text-white/50 hover:text-white transition-colors">{settings?.phone || '+234 800 000 0000'}</a>
            </li>
            <li className="flex items-center gap-2">
              <div className="w-5 h-5 rounded-md bg-white/10 flex items-center justify-center flex-shrink-0">
                <HiMail className="w-2.5 h-2.5 text-white" />
              </div>
              <a href={`mailto:${settings?.email || 'info@sidmab.com'}`} className="text-sm text-white/50 hover:text-white transition-colors">{settings?.email || 'info@sidmab.com'}</a>
            </li>
          </ul>
          {settings?.whatsapp && (
            <a
                ...
                <div className="flex gap-3 mt-6 md:mt-8">
                  {socialLinks.map(({ icon: Icon, href, color }, i) => (
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="w-10 h-10 rounded-full bg-white/10 flex items-center justify-center text-white/50 transition-all duration-300 hover:text-white hover:scale-110"
            ...
            <div className="md:hidden">
              ...
              <AnimatePresence>
                {openSection && (
                  ...
                  >
                    {sections.find((sec) => sec.id === openSection)?.content}
                  </motion.div>
                )}
              </AnimatePresence>
            </div>
